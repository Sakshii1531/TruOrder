...
export const getOrderDetails = asyncHandler(async (req, res) => {
  try {
    ...
    if (delivery.hubId && order.hubId) {
      if (delivery.hubId.toString() !== order.hubId.toString()) {
        console.warn(`⚠️ Access denied: Delivery partner ${delivery._id} (Hub: ${delivery.hubId}) tried to access order ${order.orderId} (Hub: ${order.hubId})`);
        return errorResponse(res, 403, 'This order belongs to a different hub and is not available for you.');
      }
    }
...
export const acceptOrder = asyncHandler(async (req, res) => {
  try {
    ...
    if (delivery.hubId && order.hubId) {
      if (delivery.hubId.toString() !== order.hubId.toString()) {
        console.warn(`⚠️ Access denied: Delivery partner ${delivery._id} (Hub: ${delivery.hubId}) tried to accept order ${order.orderId} (Hub: ${order.hubId})`);
        return errorResponse(res, 403, 'This order belongs to a different hub and cannot be accepted by you.');
      }
    }
