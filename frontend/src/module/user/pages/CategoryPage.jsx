    ...
    const fetchCategories = async () => {
      try {
        ...
        if (response.data && response.data.success && response.data.data && response.data.data.categories) {
          ...
          setCategories([{ id: 'all', name: "All", image: foodImages[10] || foodImages[0], slug: 'all' }])
        }
      } catch (error) {
    ...
    <div className={`min-h-screen bg-white dark:bg-[#0a0a0a] ${shouldShowGrayscale ? 'grayscale opacity-75' : ''}`}>
      <div className="sticky top-0 z-20 bg-white dark:bg-[#1a1a1a] shadow-sm">
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="flex items-center gap-2 px-4 md:px-8 py-3 border-b border-gray-100 dark:border-gray-800">
            <button
          ...
          <div
            ...
            className="flex gap-4 md:gap-6 overflow-x-auto scrollbar-hide px-4 md:px-8 py-3 bg-white dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-gray-800"
            style={{
              scrollbarWidth: "none",
              ...
              categories && categories.length > 0 ? categories.map((cat) => {
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1.5 flex-shrink-0 pb-2 transition-all ${isSelected ? 'border-b-2 border-sky-500' : ''
                      }`}
                  >
                    {cat.id === 'all' ? (
                      <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-white dark:bg-[#1a1a1a] flex items-center justify-center border-2 transition-all ${isSelected ? 'border-sky-500 shadow-lg bg-sky-50 dark:bg-sky-900/20' : 'border-slate-100 dark:border-gray-800'
                        }`}>
                        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-sky-50 dark:bg-sky-900/30 rounded-full flex items-center justify-center">
                          <UtensilsCrossed className="w-5 h-5 sm:w-6 sm:h-6 text-sky-600 dark:text-sky-400" />
                        </div>
                      </div>
                    ) : cat.image ? (
                      <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-2 transition-all ${isSelected ? 'border-sky-500 shadow-lg' : 'border-transparent'
                        }`}>
                        <img
                          src={cat.image}
                          alt={cat.name}
                      ...
                      <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center border-2 transition-all ${isSelected ? 'border-sky-500 shadow-lg bg-sky-50 dark:bg-sky-900/20' : 'border-transparent'
                        }`}>
                        <span className="text-xl md:text-2xl">🍽️</span>
                      </div>
                    )}
                    <span className={`text-xs md:text-sm font-medium whitespace-nowrap ${isSelected ? 'text-sky-600 dark:text-sky-400' : 'text-gray-600 dark:text-gray-400'
                      }`}>
                      {cat.name}
                    </span>
                  </button>
      ...
      <div className="bg-white dark:bg-[#1a1a1a] border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-[1100px] mx-auto">
          <div className="flex flex-col md:flex-row md:flex-wrap gap-2 px-4 md:px-8 py-3">
            {/* Row 1 */}
            <div
      ...
      {/* Content */}
      <div className="w-full py-4 sm:py-6 md:py-8 space-y-6 md:space-y-8">
